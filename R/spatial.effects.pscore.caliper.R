...
spatial.effects.pscore.caliper <- function(spatial.thresholds,
  ...
  trt_matrix <- matrix(treated@data$distance, length(treated),
                       length(untreated), byrow=FALSE)

  untrt_matrix <- matrix(untreated@data$distance, length(treated),
  ...
  W <- spatial.effects.distance.decay(thresh=spatial.thresholds,
                                      model=spatial.decay.model,
                                      dist=geog_dist_matrix)
